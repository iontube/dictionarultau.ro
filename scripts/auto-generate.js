...
/**
 * Auto-generate articles script for dictionarultau.ro
...
// Config - 1 article per run for dictionarultau.ro
...
async function main() {
  await log('='.repeat(60));
  await log('AUTO-GENERATE STARTED - dictionarultau.ro');
  await log('='.repeat(60));
  ...
  const projectName = process.env.CLOUDFLARE_PROJECT_NAME || 'dictionarultau-ro';
